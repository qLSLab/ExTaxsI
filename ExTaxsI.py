    ...
    # Here is how the module starts normally
        ...
        if csv_manual[0] in ("f", "F"):  # Considerare la tipologia di file da avere come input
            ...
            if plot_or_not is None:
                ...
                if len(file_list) > 2500:
                    database_module(None, 2500, file_list, menu_choice, output_name)
                ...
                if plot_or_not == "scatter":
                    return download_accession_taxonomy(counter_queries, web_env, key, search_index, directory, None)

                else:
                    return {'counter_id': counter_queries, 'webenv': web_env, 'query_key': key, 'query': search_index,
                            'folder_path': directory, 'file_input': None}

    # Otherwise search_term is a string (manual mode)
        else:
            print("|----------------------------------------------------------------------------|")
            print("|---------------------------  SEARCH FIELD TAGS  ----------------------------|")
            print("|   Accession = [ACCN]     All Fields      = [ALL]      Gene Name = [GENE]   |")
            ...
            while counter_query == 0:
                ...
                if server_errors >= 11:
                    ...
                    logging.warning("Connection impossible with NCBI, can't request %s" % search_term)
                    continue
                ...
                if server_errors >= 8:
                    print("Connection impossible with NCBI, can't request %s" % search_term)
                    logging.warning("Connection impossible with NCBI, can't request %s" % search_term)
                    continue
            ...
            if plot_or_not is None:
                ...
                return download_accession_taxonomy(counter_query, webenv, query_key, search_term, directory, None)

            elif plot_or_not == "world":
                return {'counter_id': counter_query, 'webenv': webenv, 'query_key': query_key, 'query': search_term,
                        'folder_path': directory, 'file_input': None}
...
def scatterplot():
    ...
    while True:
        ...
        if query_or_path == 2:
            ...
            plot_file_path = database_module("scatter", 0, None, None, [None, None])

        else:
            logging.info(" Getting the taxonomy file path ")
            plot_file_path = str(input(">> Enter the taxonomy file path name (only TSV or CSV files): "))
    ...
    plot_file_name = rename_file(directory, plot_file_path, '_scatterplot.html')
    filter_title = rename_file(directory, "{0}_with_less_than_{1}_results".format(plot_file_path, str(filter_value)),
                               ".txt")
    if os.path.exists(plot_file_name):
        x = 0
        try:
            overwrite = int(input(color.YELLOW +
                                  "Exist already a file with the same name, do you want to overwrite it? " +
                                  "(0 = yes, 1 = no)"
                                  + color.END))
    ...
    while os.path.exists(plot_file_name) and overwrite != 0:
        x += 1
        plot_file_name = rename_file(directory, plot_file_path, '_scatterplot_%i.html' % x)
        filter_title = rename_file(directory,
                                   "{0}_with_less_than_{1}_results".format(plot_file_path, str(filter_value)),
                                   "_%i.txt" % x)
...
def worldmap_plot():
    ...
    efetch_call_settings = database_module("world", 0, None, None, [None, None])
    ...
    if len(efetch_call_settings['query'].replace("(", "").replace(")", "")) < 15:
        print("\nThis is the automatic title: %s" % efetch_call_settings['query'].replace("(", "").replace(")", ""))

        try:
            title_choose = int(input("Do you want to keep it? (0 > yes, anything else > no) "))
    ...
    if title_choose != 0:
        ...
        title_map = efetch_call_settings['query'].replace("(", "").replace(")", "")
    ...
    update_progress(efetch_call_settings['counter_id'], efetch_call_settings['counter_id'])
    cities = []
    names = []
    element_count = 0
    print("")
...
###modify layout extaxsi:
    ...
    plot(fig, filename=rename_file(efetch_call_settings['folder_path'], title_map, "_worldmap.html"))
...
def sunburst_plot():
    ...
    while True:
        try:
            ...
            while query_or_path not in (1, 2, 3):
                print(color.RED + "Enter a valid choice, try again. \n" + color.END)
                query_or_path = input(">> Enter your choice (only the number): ")

            break

        except ValueError:
            print(color.RED + "Please enter only the numeric value" + color.END)
            print("Please enter only the numeric value")
    ...
    while True:
        ...
        if query_or_path == 2:
            plot_file_path = database_module("scatter", 0, None, None,
                                             [None, None])  # A call to download the taxonomy file and returns the path

        else:
            logging.info(" Getting the taxonomy file path ")
            plot_file_path = str(input(">> Enter the taxonomy file path name : "))
    ...
    while filter_value is None:
        ...
        try:
            filter_value = int(input('>> Enter filter value: '))
            if filter_value < 0:
                print(color.RED + "Enter a valid positive number!" + color.END)
                filter_value = None

        except ValueError:
            print(color.RED + "Enter a number!" + color.END)
    ...
    plot_file_name = rename_file(directory, title_graph, '.html')
    filter_title = rename_file(directory, "{0}_with_less_than_{1}_results".format(plot_file_path, str(filter_value)),
                               ".txt")
    if os.path.exists(plot_file_name):
        x = 0
    ...
    while os.path.exists(plot_file_name) and overwrite != 0:
        x += 1
        plot_file_name = rename_file(directory, title_graph, '_%i.html' % x)
        filter_title = rename_file(directory,
...
def main_menu():
    print("\n")
    print(color.GREEN + "      .%%%                " + color.RED + "*%%(  " + color.END)
    print(color.GREEN + "       %%%#  %%%%%%" + color.RED + "%%%%%% .%%%  " + color.END)
    print(color.GREEN + "        %%%%              " + color.RED + " %%%/ " + color.END)
    print(color.GREEN + "         #%%%#  %%%%%" + color.RED + "%%%%% %%%/ " + color.END)
    print(color.GREEN + "           %%%%%           " + color.RED + "%%%. " + color.END)
    ...
    print(color.GREEN + "                 *%%%# " + color.RED + ",%%%%    " + color.END)
    print(color.GREEN + "                     %" + color.RED + "%%%%*     " + color.END)
    print(color.RED + "                   *%%%% " + color.GREEN + "%*         " + color.CYAN +
          "-------------------------------------------------------" + color.END)
    print(color.RED + "                 %%%%%* " + color.GREEN + "*%%%        " + color.CYAN +
          "----------------- WELCOME TO " + color.BOLD + color.UNDERLINE + "ExTaxsI"+ color.END +
          color.CYAN + "! -----------------" + color.END)
    print(color.RED + "              .%%%%%     " + color.GREEN + "#%%%       " + color.CYAN +
          "-------------------------------------------------------" + color.END)
    print(color.RED + "            *%%%%#  *%%" + color.GREEN + "%  #%%%      " + color.CYAN +
          "--------- Exploration Taxonomies Informations ---------" + color.END)
    print(color.RED + "          ,%%%%*           " + color.GREEN + "%%%,     " + color.CYAN +
          "---- Tool to build and visualize amplicon database ----" + color.END)
    print(color.RED + "         %%%%*  %%%%%" + color.GREEN + "%%%%% %%%(     " + color.CYAN +
          "-------------------------------------------------------" + color.END)
    print(color.RED + "       .%%%#              " + color.GREEN + ".&%%*   " + color.END)
    print(color.RED + "      .%%%/  %%%%%" + color.GREEN + "%%%%%% ./%%%    " + color.END)
    print(color.RED + "      %%%#              " + color.GREEN + "..*%%*    " + color.END)
    print(color.RED + "      %%%  %%%%%" + color.GREEN + "%%%%% ..#%%%(     " + color.END)
    print(color.RED + "     .%%%             " + color.GREEN + "*%%%%       " + color.END)
...
if config.get('parameters', 'api_key') == "none":
    logging.info(" No api_key parameter entered, for best results enter your ncbi api key in the settings.ini file")
...
try:
    ...
    taxa_database_update = str(input("Do you want to update NCBI Taxonomy database? (Y > yes or N > no) "))
